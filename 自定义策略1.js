...
function getStockState(Account,Depth){
    var _stockBalance = Account.Stocks * Depth.Bids[0].Price;
    var _balance = Account.Balance;
    var _positionLevel = 0.0;
    if (_stockBalance + _balance != 0){
        _positionLevel = _stockBalance / (_stockBalance + _balance);
        if (_positionLevel < 0.4){
            return States.LOW;
        }else if (_positionLevel <0.7){
            return States.HALF;
        }else if (_positionLevel<=0.9){
            return States.HIGH;
        }else{
            return States.FULL;
        }
